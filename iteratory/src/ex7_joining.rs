pub fn pair_people<'a>(vec1: &'a[&'a str], vec2: &'a[&'a str]) -> Vec<(&'a &'a str, &'a &'a str)> {
    todo!()
}
...
fn test_ex7_sum_first_ten_elements() {
    ...
    assert_eq!(
        sum_first_ten_elements(&vec1, &vec2),
        10
    );
...
fn test_ex7_distance_from_position() {
    ...
    assert_eq!(
        distance_from_position(&vec1),
        vec!(0, 0, 0, 0)
    );

    let vec2 = vec!(6, 4, 1, 0);
    assert_eq!(
        distance_from_position(&vec2),
        vec!(6, 3, 1, 3)
    );
}
